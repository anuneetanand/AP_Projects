...
public class SFCode
{
    ...
    public static void ReadData() throws IOException
    {
        ...
        try
        {
            R = new FileReader("/Users/admin/IdeaProjects/SFC/input.txt");
            int c;
            while ((c = R.read()) != -1)
            {
                InputString += (char) (c);
    ...
    public static void Encode() throws FileNotFoundException
    {
        ...
        if(Data.size()==1)
        { for(int i=0;i<InputString.length();i++){EncodedString+="1";} }

        try
        {
            P1 = new PrintWriter("mapping.txt");
            if(Data.size()==1){P1.println(Data.get(0).getLetter()+":"+"1");}
            else{ Table.forEach((key, value) -> P1.println(key + ":" + value));}
        }
        finally
        { if(P1!=null) { P1.close(); } }
